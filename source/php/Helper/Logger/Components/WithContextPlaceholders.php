...
class WithContextPlaceholders extends NullLogger implements LoggerInterface
{
    ...
    public function __construct(
        ...
        $this->resolvers = $resolvers ?? [
            ...
            ['is_float',                         fn($v) => (string) $v],
            [fn($v) => $v instanceof Stringable, fn($v) => (string) $v],
            ['is_array',                         fn($v) => json_encode($v, $jsonResolverFlag ?? JSON_PRETTY_PRINT)],
            ['is_object',                        fn($v) => json_encode($v, $jsonResolverFlag ?? JSON_PRETTY_PRINT)],
        ];
    }
