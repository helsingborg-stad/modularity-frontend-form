...
describe("AND Condition", () => {
	...
	const andConditionWithoutClass = new AndCondition([
		conditionWithoutClass,
		conditionWithoutClass,
	]);

	it("getConditionFieldNames() returns an array of field names from provided Condition[]", () => {
		expect(andConditionWithoutClass.getConditionFieldNames()).toEqual([
			"testField",
			"testField",
		]);
	});
	...
	it("getConditions() returns Condition[]", () => {
		expect(andConditionWithoutClass.getConditions()).toEqual([
			conditionWithoutClass,
			conditionWithoutClass,
		]);
	});
	...
	it("validate() returns false if field is already disabled", () => {
		const andOperatorWithClass = new AndCondition([
			createConditionWithClass(true, true),
		]);
	...
	it("validate() returns false if ANY field is already disabled", () => {
		const andOperatorWithClass = new AndCondition([
			createConditionWithClass(false, true),
			createConditionWithClass(true, true),
		]);
		expect(andOperatorWithClass.validate()).toEqual(false);
	});
	...
	it("validate() returns false if field validation failed", () => {
		const andOperatorWithClass = new AndCondition([
			createConditionWithClass(false, false),
		]);
		expect(andOperatorWithClass.validate()).toEqual(false);
	});
	...
	it("validate() returns empty array if conditions are missing a name", () => {
		const andOperatorWithClass = new AndCondition([
			createConditionWithClass(false, true, null),
		]);
		expect(andOperatorWithClass.getConditionFieldNames()).toEqual([]);
	});
});

function createConditionWithClass(
	isDisabled: boolean,
	isValid: boolean,
	field: null | string = "testField",
): Condition {
	return {
		field: field,
		class: {
