class StepUIManager implements StepUIManagerInterface {
	...
	constructor(
		...
		this.nextButtonLabelElement = this.nextButton.querySelector(
	...
	public updateButtonStates(
		...
		if (this.iconElement) {
			this.iconElement.setAttribute(
				...
				activeStep === this.maxSteps ? "send" : "arrow_forward_ios",
			);
		}

		this.prevButton.classList.toggle(
			this.visibilityHiddenClass,
			activeStep === 0,
		);
	}
	 ...
	 */
	public showAndHideSteps(
		stepToShow: StepInterface,
		stepToHide: StepInterface,
	): void {
		const exitClass =
			stepToShow.getId() > stepToHide.getId()
				? this.exitLeftAnimationClass
				: this.exitRightAnimationClass;
		const allAnim = [
			this.exitLeftAnimationClass,
			this.exitRightAnimationClass,
